export type Locale = "de" | "en"

const de = {
  nav: {
    jobs: "Jobs",
    events: "Termine",
    mensa: "Mensaplan",
    infomap: "Campuskarte",
    search: "Suchen...",
    myAccount: "Mein Konto",
    login: "Einloggen",
    toggleDark: "Dunkelmodus umschalten",
    openMenu: "Menü öffnen",
    closeMenu: "Menü schließen",
    about_us: "Über uns",
    skipToContent: "Zum Inhalt springen",
    admin: "Admin",
  },
  events: {
    title: "Heutige Veranstaltungen",
    countToday: (n: number) => `${n} Veranstaltung${n !== 1 ? "en" : ""} heute`,
    showAll: "Alle anzeigen",
    none: "Keine Veranstaltungen für heute geplant.",
    clock: "Uhr",
    pageTitle: "Veranstaltungen",
    pageSubtitle: "Konzerte, Partys, Sport und mehr – von der Uni bis in die Stadt.",
    noEvents: "Keine Veranstaltungen verfügbar.",
    organizer: "Veranstalter",
    date: "Datum",
    backToEvents: "Alle Veranstaltungen",
    notFound: {
      headline: "Veranstaltung nicht gefunden",
      description:
        "Die gesuchte Veranstaltung konnte nicht gefunden werden. Möglicherweise wurde sie entfernt oder die URL ist falsch.",
    },
    moreInfo: "Weitere Informationen",
    submitEvent: "Veranstaltung einreichen",
    createTitle: "Veranstaltung einreichen",
    createSubtitle: "Die Veranstaltung wird nach einer kurzen Prüfung veröffentlicht.",
    fieldTitle: "Titel",
    fieldOrganizer: "Veranstalter",
    fieldDescription: "Beschreibung",
    fieldStart: "Beginn",
    fieldEnd: "Ende",
    fieldExternalUrl: "Link zur Veranstaltung",
    fieldCategory: "Kategorie",
    optionalLabel: "(optional)",
    categories: {
      other: "Sonstiges",
      university: "Hochschule",
      sport: "Sport",
      party: "Party",
      culture: "Kultur",
      social: "Soziales",
    },
    submitSuccess: "Veranstaltung erfolgreich eingereicht. Sie wird nach Prüfung veröffentlicht.",
    editTitle: "Veranstaltung bearbeiten",
    editEvent: "Bearbeiten",
    deleteEvent: "Löschen",
    deleteConfirm: "Veranstaltung wirklich löschen?",
    unpublishedBadge: "Depubliziert",
    rejectionReasonLabel: (reason: string) => `Grund: ${reason}`,
    updateEvent: "Änderungen speichern",
    updateSuccess: "Veranstaltung erfolgreich aktualisiert.",
    fieldLocation: "Veranstaltungsort",
    locationTypeNone: "Kein Ort",
    locationTypeLinked: "Aus Karte auswählen",
    locationTypeCustom: "Eigenen Ort eingeben",
    fieldLocationName: "Name des Orts",
    fieldLocationAddress: "Adresse",
    fieldLocationCity: "Stadt",
    locationLabel: "Ort",
    viewOnMap: "Auf Karte anzeigen",
    filterAll: "Alle",
    filterCategory: "Kategorie",
    filterDate: "Zeitraum",
    filterDateUpcoming: "Alle bevorstehenden",
    filterDateWeek: "7 Tage",
    filterDateMonth: "31 Tage",
    noEventsFiltered: "Keine Veranstaltungen für diesen Filter gefunden.",
    clearFilters: "Filter zurücksetzen",
    resultSingular: "Veranstaltung",
    resultPlural: "Veranstaltungen",
    syncedFromUnivis: "Diese Veranstaltung wurde automatisch aus UnivIS importiert.",
    syncedFromExternal:
      "Diese Veranstaltung wurde automatisch aus einer externen Quelle importiert.",
  },
  mensa: {
    cardTitle: "Mensaplan",
    cardSubtitle: (date: string) => `Morgen, ${date}`,
    cardSubtitleToday: (date: string) => `Heute, ${date}`,
    fullPlan: "Vollständiger Plan",
    noData: "Keine Daten verfügbar.",
    noDataDay: "Für diesen Tag liegen noch keine Daten vor.",
    pageLocations: "Bamberger Mensen",
    pageTitle: "Mensaplan",
    pageSubtitle: "Speisepläne für Feki, Austraße und Erba.",
    today: "Heute",
    tomorrow: "Morgen",
    weekdays: ["Sonntag", "Montag", "Dienstag", "Mittwoch", "Donnerstag", "Freitag", "Samstag"],
    weekendHeading: (sat: string, sun: string) => `Samstag & Sonntag, ${sat} – ${sun}`,
    dateLocale: "de-DE",
    vegan: "Vegan",
    vegetarian: "Vegetarisch",
  },
  jobs: {
    notFound: {
      headline: "Angebot nicht gefunden",
      description:
        "Das gesuchte Stellenangebot konnte nicht gefunden werden. Möglicherweise wurde es entfernt oder die URL ist falsch.",
    },
    title: "Aktuelle Stellenangebote",
    subtitle: "Neueste Angebote für Studenten",
    showAll: "Alle anzeigen",
    pageTitle: "Stellenangebote",
    pageSubtitle: "Aktuelle Jobs und Praktika für Studierende in Bamberg.",
    noJobs: "Zurzeit keine Stellenangebote verfügbar.",
    workingHours: (h: number) => `${h} Std./Woche`,
    contactTitle: "Kontakt",
    applyExternal: "Zur Stellenanzeige",
    backToJobs: "Alle Jobs",
    postJob: "Job inserieren",
    notPublishedHint: "Dieses Angebot ist nur für dich sichtbar.",
    rejectionReasonLabel: (reason: string) => `Grund: ${reason}`,
    editTitle: "Stellenangebot bearbeiten",
    editJob: "Bearbeiten",
    archiveJob: "Archivieren",
    archiveConfirm: "Angebot archivieren? Das kann nicht rückgängig gemacht werden.",
    deleteJob: "Löschen",
    deleteConfirm: "Stellenangebot wirklich löschen?",
    updateJob: "Änderungen speichern",
    createTitle: "Stellenangebot inserieren",
    createSubtitle: "Das Angebot wird nach einer kurzen Prüfung veröffentlicht.",
    fieldTitle: "Stellenbezeichnung",
    fieldCompany: "Unternehmen",
    fieldLocation: "Ort",
    fieldWorkingHours: "Stunden pro Woche",
    fieldDescription: "Beschreibung",
    fieldContactName: "Ansprechperson",
    fieldContactMail: "Kontakt-E-Mail",
    fieldContactPhone: "Kontakt-Telefon",
    fieldExternalUrl: "Link zur Stellenanzeige",
    fieldJobType: "Art der Stelle",
    fieldField: "Bereich",
    fieldWorkMode: "Arbeitsort",
    workModes: {
      on_site: "Vor Ort",
      hybrid: "Hybrid",
      remote: "Remote",
    },
    jobTypes: {
      part_time: "Nebenjob",
      internship: "Praktikum",
      working_student: "Werkstudent",
      research_assistant: "Hilfswissenschaftler (HiWi)",
      thesis: "Abschlussarbeit",
      volunteer: "Ehrenamt",
      other: "Sonstiges",
    },
    jobFields: {
      it: "IT & Technik",
      marketing: "Marketing & Kommunikation",
      administration: "Verwaltung & Büro",
      research: "Forschung & Lehre",
      gastronomy: "Gastronomie & Service",
      retail: "Handel & Verkauf",
      education: "Bildung & Soziales",
      other: "Sonstiges",
    },
    submitJob: "Angebot einreichen",
    submitSuccess: "Angebot erfolgreich eingereicht. Es wird nach Prüfung veröffentlicht.",
    contactSection: "Kontaktdaten",
    optionalLabel: "(optional)",
    filterHeading: "Filter & Suche",
    filterAll: "Alle",
    filterType: "Stellenart",
    filterField: "Bereich",
    filterSearch: "Nach Stelle oder Unternehmen suchen…",
    noJobsFiltered: "Keine Stellenangebote für diesen Filter gefunden.",
    noJobsFoundTitle: "Keine Stellen gefunden",
    noJobsFoundHint: "Passe die Filter oder den Suchbegriff an.",
    clearFilters: "Filter zurücksetzen",
    resultSingular: "Stellenangebot",
    resultPlural: "Stellenangebote",
    showingResults: (shown: number, total: number) => `${shown} von ${total} Ergebnissen`,
    sortNewest: "Neueste zuerst",
    sortOldest: "Älteste zuerst",
    paginationPrev: "Zurück",
    paginationNext: "Weiter",
  },
  reports: {
    reportJob: "Job melden",
    reportEvent: "Veranstaltung melden",
    modalTitle: "Inhalt melden",
    reasonLabel: "Grund",
    reasonPlaceholder: "Bitte wählen...",
    reasonSpam: "Spam",
    reasonInappropriate: "Unangemessener Inhalt",
    reasonOutdated: "Veraltet / nicht mehr aktuell",
    reasonOther: "Sonstiges",
    detailsLabel: "Details (optional)",
    detailsPlaceholder: "Optionale Erläuterung...",
    submit: "Melden",
    cancel: "Abbrechen",
    successMessage: "Meldung erfolgreich eingereicht. Danke für deinen Hinweis.",
    errorMessage: "Meldung konnte nicht eingereicht werden. Bitte versuche es erneut.",
    warningBadge: "Gemeldeter Inhalt",
    warningHint: "Achtung: Dieser Inhalt wurde mehrfach gemeldet und wird überprüft.",
    eventDisclaimer:
      "Veranstaltungen werden nicht vorab geprüft. Bitte melde unangemessene Inhalte über den Button unten.",
  },
  map: {
    pageTitle: "Campuskarte",
    pageSubtitle: "Wichtige Orte rund um die Universität Bamberg.",
    allCategories: "Alle",
    cardTitle: "Campuskarte",
    cardSubtitle: "Wichtige Orte in Bamberg",
    showMap: "Zur Campuskarte",
    upcomingEvents: "Kommende Veranstaltungen",
    eventFilterLabel: "Veranstaltungen",
    eventFilterToday: "Heute",
    eventFilterWeek: "7 Tage",
    eventFilterMonth: "31 Tage",
    eventFilterAll: "Alle",
    categories: {
      university: "Uni-Gebäude",
      mensa: "Mensa & Cafeteria",
      library: "Bibliotheken",
      sport: "Sport",
      venues: "Cafés, Bars & Clubs",
      other: "Sonstiges",
    },
  },
  locations: {
    createTitle: "Ort erstellen",
    editTitle: "Ort bearbeiten",
    fieldName: "Name",
    fieldDescription: "Beschreibung",
    fieldCategory: "Kategorie",
    fieldLat: "Breitengrad",
    fieldLon: "Längengrad",
    fieldExternalUrl: "Website",
    fieldAddressStreet: "Straße",
    fieldAddressStreetNumber: "Hausnummer",
    fieldAddressCity: "Stadt",
    fieldAddressZip: "Postleitzahl",
    optionalLabel: "(optional)",
    createLocation: "Ort erstellen",
    updateLocation: "Änderungen speichern",
    deleteLocation: "Löschen",
    deleteConfirm: "Ort wirklich löschen?",
  },
  studentGroups: {
    createTitle: "Gruppe erstellen",
    editTitle: "Gruppe bearbeiten",
    fieldName: "Name",
    fieldDescription: "Beschreibung",
    fieldWebsite: "Website",
    fieldInstagram: "Instagram",
    fieldFacebook: "Facebook",
    fieldEmail: "E-Mail",
    optionalLabel: "(optional)",
    createStudentGroup: "Gruppe erstellen",
    updateStudentGroup: "Änderungen speichern",
    deleteStudentGroup: "Löschen",
    deleteConfirm: "Gruppe wirklich löschen?",
  },
  groups: {
    title: "Aktiv in Bamberg",
    subtitle: "Studentische Gruppen und Vereine",
    description:
      "Viele Veranstaltungen, Aktivitäten und Angebote sind nur durch das ehrenamtliche Engagement von Kommilitonen möglich. Werde auch du aktiv und engagiere dich in einer der zahlreichen Gruppen, Vereine oder Initiativen an der Universität Bamberg. Hier findest du eine Übersicht über die verschiedenen Möglichkeiten, dich einzubringen und das Campus-Leben mitzugestalten.",
    sortRandom: "Zufällige Reihenfolge",
    sortByName: "Sortieren nach Name",
    showAll: "Alle Gruppen anzeigen",
    showLess: "Weniger Gruppen anzeigen",
  },
  auth: {
    loginTitle: "Einloggen",
    noAccount: "Noch kein Konto?",
    registerLink: "Registrieren",
    registerTitle: "Registrieren",
    alreadyRegistered: "Bereits registriert?",
    loginLink: "Einloggen",
    emailLabel: "E-Mail",
    emailPlaceholder: "dein.name@mailprovider.de",
    passwordLabel: "Passwort",
    passwordPlaceholder: "Passwort",
    passwordMinHint: "Mindestens 10 Zeichen",
    confirmPasswordLabel: "Passwort bestätigen",
    confirmPasswordPlaceholder: "Passwort erneut eingeben",
    loginBtn: "Einloggen",
    createAccountBtn: "Konto erstellen",
    accountTitle: "Mein Konto",
    emailFieldLabel: "E-Mail",
    memberSince: "Mitglied seit",
    logout: "Abmelden",
    dateLocale: "de-DE",
    myJobOffers: "Meine Stellenangebote",
    myEvents: "Meine Veranstaltungen",
    noMyJobOffers: "Du hast noch keine Stellenangebote inseriert.",
    noMyEvents: "Du hast noch keine Veranstaltungen eingereicht.",
    statusSubmitted: "In Prüfung",
    statusPublished: "Veröffentlicht",
    statusExpired: "Abgelaufen",
    statusRejected: "Abgelehnt",
    statusArchived: "Archiviert",
    confirmSuccess: "E-Mail erfolgreich bestätigt! Du wirst weitergeleitet…",
    confirmFailed: "Bestätigungslink ungültig oder abgelaufen.",
    emailConfirmed: "E-Mail-Adresse bestätigt!",
    emailConfirmedYouCanLogin:
      "Deine E-Mail-Adresse wurde erfolgreich bestätigt. Du kannst dich jetzt anmelden.",
    verifyEmailRequiredTitle: "E-Mail-Adresse nicht bestätigt",
    verifyEmailRequiredText:
      "Bitte bestätige deine E-Mail-Adresse, um Stellenangebote einreichen zu können.",
    resendVerificationBtn: "Bestätigungs-E-Mail erneut senden",
    resendVerificationSent: "E-Mail wurde gesendet",
    checkYourEmailTitle: "Bestätige deine E-Mail-Adresse",
    checkYourEmailText:
      "Wir haben dir eine E-Mail mit einem Bestätigungslink geschickt. Bitte klicke auf den Link, um dein Konto zu aktivieren.",
    forgotPasswordTitle: "Passwort vergessen",
    forgotPasswordInstruction:
      "Gib deine E-Mail-Adresse ein. Falls ein Konto mit dieser Adresse existiert, senden wir dir einen Link zum Zurücksetzen deines Passworts.",
    forgotPasswordBtn: "Link anfordern",
    forgotPasswordSuccess:
      "Falls ein Konto mit dieser E-Mail-Adresse existiert, wurde ein Link zum Zurücksetzen des Passworts gesendet.",
    forgotPasswordLink: "Passwort vergessen?",
    resetPasswordTitle: "Neues Passwort festlegen",
    resetPasswordInstruction: "Gib dein neues Passwort ein.",
    resetPasswordBtn: "Passwort speichern",
    resetPasswordSuccess: "Passwort erfolgreich geändert. Du kannst dich jetzt anmelden.",
    resetPasswordNewLabel: "Neues Passwort",
    resetPasswordConfirmLabel: "Passwort bestätigen",
    resetPasswordInvalidCode: "Der Reset-Link ist ungültig oder abgelaufen.",
    validation: {
      emailInvalid: "Bitte gültige E-Mail eingeben.",
      passwordRequired: "Bitte Passwort eingeben.",
      passwordMismatch: "Die Passwörter stimmen nicht überein.",
      loginFailed: "Anmeldung fehlgeschlagen.",
      registerFailed: "Registrierung fehlgeschlagen.",
      resetFailed: "Passwort konnte nicht zurückgesetzt werden.",
    },
  },
  ui: {
    cancel: "Abbrechen",
  },
  cookieBanner: {
    text: "Diese Website verwendet ausschließlich technisch notwendige Cookies für Anmeldung und Spracheinstellungen. Weitere Informationen findest du in unserer ",
    privacyLink: "Datenschutzerklärung",
    dismiss: "Verstanden",
  },
  footer: {
    tagline: "Das Studierendenportal für Bamberg.",
    impressum: "Impressum",
    privacyPolicy: "Datenschutz",
  },
  about: {
    pageTitle: "Über uns",
    pageSubtitle: "Das studentische Informationsportal für Bamberg.",
    missionHeading: "Was ist BamBase.de?",
    missionText:
      "BamBase.de bündelt Informationen rund um das Studium an der Universität Bamberg an einem zentralen Ort: Veranstaltungen, Stellenangebote, Mensapläne, wichtige Orte auf dem Campus und studentische Gruppen – alles auf einen Blick.",
    contributingHeading: "Mitmachen",
    contributingText:
      "Du kannst Veranstaltungen und Stellenangebote direkt über die Website einreichen. Sie werden nach einer kurzen Prüfung veröffentlicht. Studentische Gruppen, die ihre Informationen ergänzen möchten, wenden sich bitte an das Team.",
    openSourceHeading: "Open Source",
    openSourceText:
      "BamBase.de ist ein Open-Source-Projekt. Fehler, Verbesserungsvorschläge und Fragen können auf GitHub als Issue gemeldet werden.",
    openSourceLink: "Zum GitHub-Repository",
    legalHeading: "Rechtliches",
    legalText: "Informationen zu Betreiber, Datenschutz und rechtlichen Hinweisen findest du im",
    legalLink: "Impressum",
  },
  impressum: {
    pageTitle: "Impressum",
    pageSubtitle: "Rechtliche Angaben zum Betreiber dieses Angebots.",
  },
  privacyPolicy: {
    pageTitle: "Datenschutzerklärung",
    pageSubtitle: "Informationen zur Verarbeitung personenbezogener Daten.",
  },
  page404: {
    pageTitle: "Seite nicht gefunden",
    headline: "404",
    description: "Die gesuchte Seite existiert nicht oder wurde verschoben.",
    backHome: "Zur Startseite",
  },
  page500: {
    pageTitle: "Serverfehler",
    headline: "500",
    description: "Ein unerwarteter Fehler ist aufgetreten. Bitte versuche es später erneut.",
    backHome: "Zur Startseite",
  },
  apiDown: {
    message:
      "Der Service ist momentan nicht erreichbar. Einige Inhalte werden möglicherweise nicht angezeigt.",
  },
  language: {
    switchLabel: "EN",
    switchAccessibleLabel: "Switch to English",
    switchLang: "en" as Locale,
  },
  admin: {
    overviewTitle: "Admin-Übersicht",
    overviewJobsDescription: "Eingereichte Stellenangebote prüfen und freigeben.",
    overviewEventsDescription: "Veranstaltungen einsehen und bearbeiten.",
    overviewLocationsDescription: "Orte auf der Campuskarte verwalten.",
    overviewStudentGroupsDescription: "Studentische Gruppen und Vereine verwalten.",
    overviewUsersDescription: "Nutzerkonten verwalten und sperren.",
    overviewReportsDescription: "Gemeldete Veranstaltungen und Stellenangebote prüfen.",
    jobsQueueTitle: "Stellenangebote prüfen",
    noSubmittedJobs: "Keine Stellenangebote warten aktuell auf Prüfung.",
    editJob: "Bearbeiten",
    approveJob: "Genehmigen",
    rejectJob: "Ablehnen",
    recentDecisionsTitle: "Zuletzt entschieden",
    noRecentDecisions: "Es wurden noch keine Stellenangebote entschieden.",
    decidedAt: (date: string) => `Entschieden am ${date}`,
    eventsTitle: "Veranstaltungen verwalten",
    noEvents: "Es sind keine Veranstaltungen vorhanden.",
    editEvent: "Bearbeiten",
    locationsTitle: "Orte verwalten",
    noLocations: "Es sind keine Orte vorhanden.",
    newLocation: "Neuer Ort",
    editLocation: "Bearbeiten",
    studentGroupsTitle: "Gruppen verwalten",
    noStudentGroups: "Es sind keine Gruppen vorhanden.",
    newStudentGroup: "Neue Gruppe",
    editStudentGroup: "Bearbeiten",
    usersTitle: "Nutzerverwaltung",
    usersSubtitle: "Zugriff, Rollen und Kontoverifizierung für die Community verwalten.",
    noUsers: "Keine Nutzer gefunden.",
    roleLabel: "Rolle",
    statusActive: "Aktiv",
    statusBanned: "Gesperrt",
    statusVerified: "Verifiziert",
    statusUnverified: "Nicht verifiziert",
    reasonPlaceholder: "Grund (optional)",
    banAction: "Sperren",
    unbanAction: "Entsperren",
    bannedUntil: (date: string) => `Gesperrt bis ${date}`,
    bannedIndefinitely: "Dauerhaft gesperrt",
    bannedYou: "(du)",
    statTotalUsers: "Nutzer insgesamt",
    statVerifiedAccounts: "Verifizierte Konten",
    statBannedAccounts: "Gesperrte Konten",
    searchPlaceholder: "Nach Name oder E-Mail suchen…",
    applyFilters: "Filtern",
    filterAllRoles: "Alle Rollen",
    filterAllStatuses: "Alle Status",
    filterAllVerification: "Alle E-Mails",
    usersShownCount: (n: number) => `${n} Nutzer angezeigt`,
    noUsersMatchFilters: "Keine Nutzer entsprechen den aktuellen Filtern.",
    clearFilters: "Filter zurücksetzen",
    columnUser: "Nutzer",
    columnRole: "Rolle",
    columnVerification: "Verifizierung",
    columnStatus: "Status",
    columnLastActive: "Zuletzt aktiv",
    columnActions: "Aktionen",
    neverActiveLabel: "Noch nie aktiv",
    roleNames: {
      user: "Nutzer:in",
      jobModerator: "Job-Moderation",
      eventModerator: "Termin-Moderation",
      admin: "Administrator:in",
    },
    changeRoleLabel: (name: string) => `Rolle von ${name} ändern`,
    inviteUser: "Nutzer:in einladen",
    inviteUserSubtitle: "Erstellt ein Konto und sendet eine E-Mail zum Festlegen des Passworts.",
    inviteNameLabel: "Name",
    inviteEmailLabel: "E-Mail-Adresse",
    inviteRoleLabel: "Rolle",
    inviteSubmit: "Einladung senden",
    inviteSuccessMessage: "Die Einladung wurde per E-Mail verschickt.",
    reportsTitle: "Meldungen prüfen",
    noReports: "Keine Meldungen in dieser Ansicht.",
    reportsStatusOpen: "Offen",
    reportsStatusDismissed: "Verworfen",
    reportsStatusResolved: "Erledigt",
    reportsStatusAll: "Alle",
    reportsTargetMissing: "Inhalt nicht mehr verfügbar",
    reportsDismiss: "Verwerfen",
    reportsReopen: "Wiedereröffnen",
    reportsDismissAll: "Alle verwerfen",
    reportsReopenAll: "Alle wiedereröffnen",
    reportsCount: (n: number) => `${n} Meldung${n !== 1 ? "en" : ""}`,
    reportsOpenDismissedSummary: (open: number, dismissed: number) =>
      `${open} offen · ${dismissed} verworfen`,
    reportsLatestAt: (date: string) => `Zuletzt gemeldet am ${date}`,
    reportsShowIndividual: (n: number) => `Einzelne Meldungen anzeigen (${n})`,
    reportsMultiple: "Mehrfach gemeldet",
    reportsUnpublish: "Depublizieren",
    reportsPublish: "Veröffentlichen",
    reportsUnpublished: "Depubliziert",
    reportsEventsTitle: "Gemeldete Veranstaltungen",
    reportsEventsDescription: "Mehrfach gemeldete Veranstaltungen prüfen.",
    reportsJobsTitle: "Gemeldete Stellenangebote",
    reportsJobsDescription: "Mehrfach gemeldete Stellenangebote prüfen.",
    reportsOpenGroupsCount: (n: number) =>
      `${n} ${n === 1 ? "offener Eintrag" : "offene Einträge"}`,
    overviewCronDescription: "Status der geplanten Hintergrundjobs einsehen.",
    cronTitle: "Hintergrundjobs",
    cronJobLabels: {
      "mensa-sync": "Mensa-Sync",
      "event-sync": "UniVis-Veranstaltungs-Sync",
      "job-offer-sync": "Stellenangebote-Sync",
      "job-offer-expiry": "Stellenangebote-Ablauf",
    },
    cronScheduleLabel: (schedule: string) => `Zeitplan: ${schedule}`,
    cronNeverRun: "Noch nie ausgeführt.",
    cronLastRunAt: (date: string) => `Zuletzt ausgeführt am ${date}`,
    cronDuration: (seconds: string) => `Dauer: ${seconds}s`,
    cronStatusSuccess: "Erfolgreich",
    cronStatusError: "Fehlgeschlagen",
    cronStatusNeverRun: "Noch nie ausgeführt",
    cronRunNow: "Jetzt ausführen",
  },
}

const en: typeof de = {
  nav: {
    jobs: "Jobs",
    events: "Events",
    mensa: "Cafeteria",
    infomap: "Campus Map",
    search: "Search...",
    myAccount: "My Account",
    login: "Log in",
    toggleDark: "Toggle dark mode",
    openMenu: "Open menu",
    closeMenu: "Close menu",
    about_us: "About us",
    skipToContent: "Skip to content",
    admin: "Admin",
  },
  events: {
    title: "Today's Events",
    countToday: (n: number) => `${n} event${n !== 1 ? "s" : ""} today`,
    showAll: "Show all",
    none: "No events scheduled for today.",
    clock: "",
    pageTitle: "Events",
    pageSubtitle: "Concerts, parties, sports and more – from campus to the city.",
    noEvents: "No events available.",
    organizer: "Organizer",
    date: "Date",
    backToEvents: "All events",
    notFound: {
      headline: "Event not found",
      description:
        "The event you are looking for could not be found. It may have been removed or the URL is incorrect.",
    },
    moreInfo: "More information",
    submitEvent: "Submit event",
    createTitle: "Submit an event",
    createSubtitle: "Your event will be reviewed before it goes live.",
    fieldTitle: "Title",
    fieldOrganizer: "Organizer",
    fieldDescription: "Description",
    fieldStart: "Start",
    fieldEnd: "End",
    fieldExternalUrl: "Link to event",
    fieldCategory: "Category",
    optionalLabel: "(optional)",
    categories: {
      other: "Other",
      university: "University",
      sport: "Sports",
      party: "Party",
      culture: "Culture",
      social: "Social",
    },
    submitSuccess: "Event submitted successfully. It will be published after review.",
    editTitle: "Edit event",
    editEvent: "Edit",
    deleteEvent: "Delete",
    deleteConfirm: "Really delete this event?",
    unpublishedBadge: "Unpublished",
    rejectionReasonLabel: (reason: string) => `Reason: ${reason}`,
    updateEvent: "Save changes",
    updateSuccess: "Event updated successfully.",
    fieldLocation: "Venue",
    locationTypeNone: "No location",
    locationTypeLinked: "Select from map",
    locationTypeCustom: "Enter custom location",
    fieldLocationName: "Venue name",
    fieldLocationAddress: "Address",
    fieldLocationCity: "City",
    locationLabel: "Location",
    viewOnMap: "View on map",
    filterAll: "All",
    filterCategory: "Category",
    filterDate: "Time range",
    filterDateUpcoming: "All upcoming",
    filterDateWeek: "7 days",
    filterDateMonth: "31 days",
    noEventsFiltered: "No events match this filter.",
    clearFilters: "Clear filters",
    resultSingular: "event",
    resultPlural: "events",
    syncedFromUnivis: "This event was automatically imported from UnivIS.",
    syncedFromExternal: "This event was automatically imported from an external source.",
  },
  mensa: {
    cardTitle: "Cafeteria",
    cardSubtitle: (date: string) => `Tomorrow, ${date}`,
    cardSubtitleToday: (date: string) => `Today, ${date}`,
    fullPlan: "Full schedule",
    noData: "No data available.",
    noDataDay: "No data available for this day yet.",
    pageLocations: "Bamberg Cafeterias",
    pageTitle: "Cafeteria Plan",
    pageSubtitle: "Menus for Feki, Austraße and Erba.",
    today: "Today",
    tomorrow: "Tomorrow",
    weekdays: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"],
    weekendHeading: (sat: string, sun: string) => `Saturday & Sunday, ${sat} – ${sun}`,
    dateLocale: "en-US",
    vegan: "Vegan",
    vegetarian: "Vegetarian",
  },
  jobs: {
    notFound: {
      headline: "Offer not found",
      description:
        "The job offer you are looking for could not be found. It may have been removed or the URL is incorrect.",
    },
    title: "Recent Job Listings",
    subtitle: "Latest offers for students",
    showAll: "Show all",
    pageTitle: "Job Listings",
    pageSubtitle: "Recent jobs and internships for students in Bamberg.",
    noJobs: "No job listings available at the moment.",
    workingHours: (h: number) => `${h} hrs/week`,
    contactTitle: "Contact",
    applyExternal: "View listing",
    backToJobs: "All jobs",
    postJob: "Post a job",
    notPublishedHint: "This listing is only visible to you.",
    rejectionReasonLabel: (reason: string) => `Reason: ${reason}`,
    editTitle: "Edit job listing",
    editJob: "Edit",
    archiveJob: "Archive",
    archiveConfirm: "Archive this job listing? This cannot be undone.",
    deleteJob: "Delete",
    deleteConfirm: "Really delete this job listing?",
    updateJob: "Save changes",
    createTitle: "Post a job listing",
    createSubtitle: "Your listing will be reviewed before it goes live.",
    fieldTitle: "Job title",
    fieldCompany: "Company",
    fieldLocation: "Location",
    fieldWorkingHours: "Hours per week",
    fieldDescription: "Description",
    fieldContactName: "Contact person",
    fieldContactMail: "Contact email",
    fieldContactPhone: "Contact phone",
    fieldExternalUrl: "Link to job listing",
    fieldJobType: "Job type",
    fieldField: "Field",
    fieldWorkMode: "Work mode",
    workModes: {
      on_site: "On-site",
      hybrid: "Hybrid",
      remote: "Remote",
    },
    jobTypes: {
      part_time: "Part-time job",
      internship: "Internship",
      working_student: "Working student",
      research_assistant: "Research assistant (HiWi)",
      thesis: "Thesis",
      volunteer: "Volunteer",
      other: "Other",
    },
    jobFields: {
      it: "IT & Technology",
      marketing: "Marketing & Communications",
      administration: "Administration & Office",
      research: "Research & Teaching",
      gastronomy: "Gastronomy & Service",
      retail: "Retail & Sales",
      education: "Education & Social Work",
      other: "Other",
    },
    submitJob: "Submit listing",
    submitSuccess: "Listing submitted successfully. It will be published after review.",
    contactSection: "Contact details",
    optionalLabel: "(optional)",
    filterHeading: "Filter & Search",
    filterAll: "All",
    filterType: "Job type",
    filterField: "Field",
    filterSearch: "Search by title or company…",
    noJobsFiltered: "No job listings match this filter.",
    noJobsFoundTitle: "No jobs found",
    noJobsFoundHint: "Try adjusting your filters or search term.",
    clearFilters: "Clear filters",
    resultSingular: "job listing",
    resultPlural: "job listings",
    showingResults: (shown: number, total: number) => `Showing ${shown} of ${total} results`,
    sortNewest: "Newest first",
    sortOldest: "Oldest first",
    paginationPrev: "Previous",
    paginationNext: "Next",
  },
  reports: {
    reportJob: "Report listing",
    reportEvent: "Report event",
    modalTitle: "Report content",
    reasonLabel: "Reason",
    reasonPlaceholder: "Please select...",
    reasonSpam: "Spam",
    reasonInappropriate: "Inappropriate content",
    reasonOutdated: "Outdated / no longer accurate",
    reasonOther: "Other",
    detailsLabel: "Details (optional)",
    detailsPlaceholder: "Optional explanation...",
    submit: "Submit report",
    cancel: "Cancel",
    successMessage: "Report submitted successfully. Thank you for your feedback.",
    errorMessage: "Report could not be submitted. Please try again.",
    warningBadge: "Reported content",
    warningHint: "Attention: This content has been reported multiple times and is under review.",
    eventDisclaimer:
      "Events are not subject to prior approval. Please report any inappropriate content using the button below.",
  },
  map: {
    pageTitle: "Campus Map",
    pageSubtitle: "Important locations around the University of Bamberg.",
    allCategories: "All",
    cardTitle: "Campus Map",
    cardSubtitle: "Important locations in Bamberg",
    showMap: "Open campus map",
    upcomingEvents: "Upcoming events",
    eventFilterLabel: "Show Events",
    eventFilterToday: "Today",
    eventFilterWeek: "7 days",
    eventFilterMonth: "31 days",
    eventFilterAll: "All",
    categories: {
      university: "University",
      mensa: "Cafeteria",
      library: "Library",
      sport: "Sports",
      venues: "Cafés, Bars & Clubs",
      other: "Other",
    },
  },
  locations: {
    createTitle: "Create location",
    editTitle: "Edit location",
    fieldName: "Name",
    fieldDescription: "Description",
    fieldCategory: "Category",
    fieldLat: "Latitude",
    fieldLon: "Longitude",
    fieldExternalUrl: "Website",
    fieldAddressStreet: "Street",
    fieldAddressStreetNumber: "House number",
    fieldAddressCity: "City",
    fieldAddressZip: "Zip code",
    optionalLabel: "(optional)",
    createLocation: "Create location",
    updateLocation: "Save changes",
    deleteLocation: "Delete",
    deleteConfirm: "Really delete this location?",
  },
  studentGroups: {
    createTitle: "Create group",
    editTitle: "Edit group",
    fieldName: "Name",
    fieldDescription: "Description",
    fieldWebsite: "Website",
    fieldInstagram: "Instagram",
    fieldFacebook: "Facebook",
    fieldEmail: "Email",
    optionalLabel: "(optional)",
    createStudentGroup: "Create group",
    updateStudentGroup: "Save changes",
    deleteStudentGroup: "Delete",
    deleteConfirm: "Really delete this group?",
  },
  groups: {
    title: "Active in Bamberg",
    subtitle: "Student groups and associations",
    description:
      "Many events, activities and services are only possible through the voluntary commitment of fellow students. Get involved and join one of the many groups, associations or initiatives at the University of Bamberg. Here you will find an overview of the various ways to contribute and help shape campus life.",
    sortRandom: "Random order",
    sortByName: "Sort by name",
    showAll: "Show all groups",
    showLess: "Show fewer groups",
  },
  auth: {
    loginTitle: "Log in",
    noAccount: "Don't have an account?",
    registerLink: "Register",
    registerTitle: "Register",
    alreadyRegistered: "Already registered?",
    loginLink: "Log in",
    emailLabel: "Email",
    emailPlaceholder: "your.name@mailprovider.com",
    passwordLabel: "Password",
    passwordPlaceholder: "Password",
    passwordMinHint: "At least 10 characters",
    confirmPasswordLabel: "Confirm password",
    confirmPasswordPlaceholder: "Enter password again",
    loginBtn: "Log in",
    createAccountBtn: "Create account",
    accountTitle: "My Account",
    emailFieldLabel: "Email",
    memberSince: "Member since",
    logout: "Log out",
    dateLocale: "en-US",
    myJobOffers: "My job listings",
    myEvents: "My events",
    noMyJobOffers: "You haven't posted any job listings yet.",
    noMyEvents: "You haven't submitted any events yet.",
    statusSubmitted: "Under review",
    statusPublished: "Published",
    statusExpired: "Expired",
    statusRejected: "Rejected",
    statusArchived: "Archived",
    confirmSuccess: "Email confirmed successfully! Redirecting…",
    confirmFailed: "Confirmation link is invalid or has expired.",
    emailConfirmed: "Email address confirmed!",
    emailConfirmedYouCanLogin:
      "Your email address has been successfully confirmed. You can now log in.",
    verifyEmailRequiredTitle: "Email address not verified",
    verifyEmailRequiredText: "Please verify your email address to be able to submit job listings.",
    resendVerificationBtn: "Resend verification email",
    resendVerificationSent: "Email sent",
    checkYourEmailTitle: "Confirm your email address",
    checkYourEmailText:
      "We've sent you an email with a confirmation link. Please click the link to activate your account.",
    forgotPasswordTitle: "Forgot password",
    forgotPasswordInstruction:
      "Enter your email address. If an account exists for that address, we will send you a link to reset your password.",
    forgotPasswordBtn: "Send reset link",
    forgotPasswordSuccess:
      "If an account exists for that email address, a password reset link has been sent.",
    forgotPasswordLink: "Forgot password?",
    resetPasswordTitle: "Set new password",
    resetPasswordInstruction: "Enter your new password.",
    resetPasswordBtn: "Save password",
    resetPasswordSuccess: "Password changed successfully. You can now log in.",
    resetPasswordNewLabel: "New password",
    resetPasswordConfirmLabel: "Confirm password",
    resetPasswordInvalidCode: "The reset link is invalid or has expired.",
    validation: {
      emailInvalid: "Please enter a valid email address.",
      passwordRequired: "Please enter your password.",
      passwordMismatch: "Passwords do not match.",
      loginFailed: "Login failed.",
      registerFailed: "Registration failed.",
      resetFailed: "Password could not be reset.",
    },
  },
  ui: {
    cancel: "Cancel",
  },
  cookieBanner: {
    text: "This website uses only essential cookies for authentication and language settings. For more information, see our ",
    privacyLink: "Privacy Policy",
    dismiss: "Got it",
  },
  footer: {
    tagline: "The student portal for Bamberg.",
    impressum: "Legal Notice",
    privacyPolicy: "Privacy Policy",
  },
  about: {
    pageTitle: "About us",
    pageSubtitle: "The student information portal for Bamberg.",
    missionHeading: "What is BamBase.de?",
    missionText:
      "BamBase.de brings together information about student life at the University of Bamberg in one place: events, job listings, cafeteria menus, important campus locations, and student groups – all at a glance.",
    contributingHeading: "Get involved",
    contributingText:
      "You can submit events and job listings directly through the website. They will be published after a short review. Student groups that want to add or update their information are welcome to reach out to the team.",
    openSourceHeading: "Open Source",
    openSourceText:
      "BamBase.de is an open-source project. Bugs, feature requests, and questions can be reported as issues on GitHub.",
    openSourceLink: "View on GitHub",
    legalHeading: "Legal",
    legalText:
      "Information about the operator, privacy policy, and legal notices can be found in the",
    legalLink: "Legal Notice",
  },
  impressum: {
    pageTitle: "Legal Notice",
    pageSubtitle: "Legal information about the operator of this service.",
  },
  privacyPolicy: {
    pageTitle: "Privacy Policy",
    pageSubtitle: "Information on the processing of personal data.",
  },
  page404: {
    pageTitle: "Page not found",
    headline: "404",
    description: "The page you are looking for does not exist or has been moved.",
    backHome: "Back to homepage",
  },
  page500: {
    pageTitle: "Server error",
    headline: "500",
    description: "An unexpected error occurred. Please try again later.",
    backHome: "Back to homepage",
  },
  apiDown: {
    message: "The service is currently unavailable. Some content may not be displayed.",
  },
  language: {
    switchLabel: "DE",
    switchAccessibleLabel: "Zu Deutsch wechseln",
    switchLang: "de" as Locale,
  },
  admin: {
    overviewTitle: "Admin overview",
    overviewJobsDescription: "Review and approve submitted job offers.",
    overviewEventsDescription: "View and edit events.",
    overviewLocationsDescription: "Manage locations on the campus map.",
    overviewStudentGroupsDescription: "Manage student groups and associations.",
    overviewUsersDescription: "Manage and ban user accounts.",
    overviewReportsDescription: "Review reported events and job offers.",
    jobsQueueTitle: "Review job offers",
    noSubmittedJobs: "No job offers are currently awaiting review.",
    editJob: "Edit",
    approveJob: "Approve",
    rejectJob: "Reject",
    recentDecisionsTitle: "Recently decided",
    noRecentDecisions: "No job offers have been decided yet.",
    decidedAt: (date: string) => `Decided on ${date}`,
    eventsTitle: "Manage events",
    noEvents: "There are no events yet.",
    editEvent: "Edit",
    locationsTitle: "Manage locations",
    noLocations: "There are no locations yet.",
    newLocation: "New location",
    editLocation: "Edit",
    studentGroupsTitle: "Manage groups",
    noStudentGroups: "There are no groups yet.",
    newStudentGroup: "New group",
    editStudentGroup: "Edit",
    usersTitle: "User management",
    usersSubtitle: "Manage access, roles, and account verification for the community.",
    noUsers: "No users found.",
    roleLabel: "Role",
    statusActive: "Active",
    statusBanned: "Banned",
    statusVerified: "Verified",
    statusUnverified: "Unverified",
    reasonPlaceholder: "Reason (optional)",
    banAction: "Ban",
    unbanAction: "Unban",
    bannedUntil: (date: string) => `Banned until ${date}`,
    bannedIndefinitely: "Banned indefinitely",
    bannedYou: "(you)",
    statTotalUsers: "Total users",
    statVerifiedAccounts: "Verified accounts",
    statBannedAccounts: "Banned accounts",
    searchPlaceholder: "Search by name or email…",
    applyFilters: "Apply filters",
    filterAllRoles: "All roles",
    filterAllStatuses: "All statuses",
    filterAllVerification: "All emails",
    usersShownCount: (n: number) => `${n} user${n === 1 ? "" : "s"} shown`,
    noUsersMatchFilters: "No users match the current filters.",
    clearFilters: "Clear filters",
    columnUser: "User",
    columnRole: "Role",
    columnVerification: "Verification",
    columnStatus: "Status",
    columnLastActive: "Last active",
    columnActions: "Actions",
    neverActiveLabel: "Never active",
    roleNames: {
      user: "User",
      jobModerator: "Job moderator",
      eventModerator: "Event moderator",
      admin: "Administrator",
    },
    changeRoleLabel: (name: string) => `Change role of ${name}`,
    inviteUser: "Invite user",
    inviteUserSubtitle: "Creates an account and emails a link to set a password.",
    inviteNameLabel: "Name",
    inviteEmailLabel: "Email address",
    inviteRoleLabel: "Role",
    inviteSubmit: "Send invite",
    inviteSuccessMessage: "The invitation email has been sent.",
    reportsTitle: "Review reports",
    noReports: "No reports in this view.",
    reportsStatusOpen: "Open",
    reportsStatusDismissed: "Dismissed",
    reportsStatusResolved: "Resolved",
    reportsStatusAll: "All",
    reportsTargetMissing: "Content no longer available",
    reportsDismiss: "Dismiss",
    reportsReopen: "Reopen",
    reportsDismissAll: "Dismiss all",
    reportsReopenAll: "Reopen all",
    reportsCount: (n: number) => `${n} report${n !== 1 ? "s" : ""}`,
    reportsOpenDismissedSummary: (open: number, dismissed: number) =>
      `${open} open · ${dismissed} dismissed`,
    reportsLatestAt: (date: string) => `Last reported on ${date}`,
    reportsShowIndividual: (n: number) => `Show individual reports (${n})`,
    reportsMultiple: "Reported multiple times",
    reportsUnpublish: "Unpublish",
    reportsPublish: "Publish",
    reportsUnpublished: "Unpublished",
    reportsEventsTitle: "Reported events",
    reportsEventsDescription: "Review events that have been reported multiple times.",
    reportsJobsTitle: "Reported jobs",
    reportsJobsDescription: "Review job offers that have been reported multiple times.",
    reportsOpenGroupsCount: (n: number) => `${n} open ${n === 1 ? "entry" : "entries"}`,
    overviewCronDescription: "View the status of scheduled background jobs.",
    cronTitle: "Background jobs",
    cronJobLabels: {
      "mensa-sync": "Cafeteria sync",
      "event-sync": "UniVis event sync",
      "job-offer-sync": "Job offer sync",
      "job-offer-expiry": "Job offer expiry",
    },
    cronScheduleLabel: (schedule: string) => `Schedule: ${schedule}`,
    cronNeverRun: "Never run yet.",
    cronLastRunAt: (date: string) => `Last run on ${date}`,
    cronDuration: (seconds: string) => `Duration: ${seconds}s`,
    cronStatusSuccess: "Success",
    cronStatusError: "Failed",
    cronStatusNeverRun: "Never run",
    cronRunNow: "Run now",
  },
}

const translations = { de, en }

export function useTranslations(locale: Locale) {
  return translations[locale]
}
