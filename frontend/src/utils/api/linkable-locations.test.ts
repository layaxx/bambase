...
import { fetchLinkableLocations } from "./linkable-locations"
...
describe("fetchLinkableLocations", () => {
  ...
  it("sends a category $eq filter when a category is provided", async () => {
    ...
    await fetchLinkableLocations("university")
  ...
  it("returns the data array from the response", async () => {
    ...
    const result = await fetchLinkableLocations()
  ...
  it("logs an error and returns an empty array when the API response is unexpected", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "Error fetching linkable locations",
