...
Migrated from bare `@fontsource-variable` CSS imports to Astro's native `fonts` API (stable since Astro 6.0). Both `@fontsource-variable/archivo` and `@fontsource-variable/inter` are removed from `package.json`; Astro now fetches, caches, and serves fonts from its own `_astro/fonts/` directory.

- `astro.config.mjs`: added `fonts` array using `fontProviders.fontsource()` — Inter as a variable font (`weights: ["100 900"]`, normal), Archivo as static weight 800 only (`weights: [800]`, normal only).
- `Layout.astro`: replaced bare font imports with `<Font cssVariable="--font-inter" preload />` and `<Font cssVariable="--font-archivo" preload />`, which inject `<link rel="preload" as="font" crossorigin>` tags and `@font-face` CSS per request.
- `global.css`: updated `@theme { --font-sans }` and `.logo-font` to reference the CSS variables (`var(--font-inter)` / `var(--font-archivo)`) generated by the Font components.
- `font-display: swap` is handled by Astro's fonts API by default (no manual override needed).
- `@fontsource/inter` kept as a direct dependency — `render.ts` reads it via `fs.readFile` for Satori OG image generation; that code path is unrelated to CSS font loading.
- Archivo font file: 35 KB variable → 15 KB static 800, a 57% size reduction.
